...
func newMessenger(cfg config.Config) EventMessenger {
	...
	if cfg.SHMAdminBaseURL != "" {
		shmClient := shm.NewClient(cfg.SHMAdminBaseURL, cfg.SHMAdminLogin, cfg.SHMAdminPassword, cfg.SHMRequestTimeout)
		categoryResolver = shm.NewCachedCategoryLookup(shmClient, cfg.SHMServiceCategoryCacheTTL)
	}
